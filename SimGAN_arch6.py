...
class Discriminator(nn.Module):
    def __init__(self): #, aux_cls = True
        ...
        self.downsample = nn.Sequential(
                nn.Conv2d(1, 32, 3, 2, 0), # 32*17*27
                nn.BatchNorm2d(32),
                nn.LeakyReLU()
        )

        self.fc = nn.Sequential(
                nn.Linear(32*27*17, 1000),
                nn.BatchNorm1d(1000),
                nn.LeakyReLU(),
                
                nn.Linear(1000, 32*27*17),
                nn.BatchNorm1d(32*27*17),
                nn.LeakyReLU()
        )
        # Upsampling
        self.upsample = nn.Sequential(
                nn.ConvTranspose2d(32, 32, 3, 2, 0), # 32*35*55
                nn.BatchNorm2d(32),
                nn.LeakyReLU(),
    ...
    def forward(self, x):
        ...
        #print(out.shape)
        out = self.upsample(out.view(out.size(0), 32, 17, 27))
        #print(out.shape)
        #print('='*20)
        '''
        if self.aux_cls :
