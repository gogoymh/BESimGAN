...
from SimGAN_arch3 import Generator, Discriminator
...
parser.add_argument("--exp", type=int, default=37, help="number of experiment")
...
#parser.add_argument("--delta", type=float, default=0.5, help="Scale factor of refine loss")
...
synthetic_data = fn.synthetic2_loader("C:/유민형/개인 연구/BESimGAN/Dataset/UnityEyes_Windows/imgs",
                               transform=transforms.Compose([
                                       transforms.CenterCrop((180,250)),
                                       transforms.Resize((35,55)),
                                       transforms.ToTensor(),
                                       transforms.Normalize((0.4117,),(0.2069,))]))

synthetic = DataLoader(synthetic_data, batch_size=opt.batch_size, shuffle=True)

real_data = fn.real_loader("C:/유민형/개인 연구/BESimGAN/Dataset/real4",
                            transform=transforms.Compose([
                                    transforms.ToTensor(),
                                    transforms.Normalize((0.5,),(0.5,))]))

real = DataLoader(real_data, batch_size=opt.batch_size, shuffle=True)
...
for epoch in range(opt.n_epochs):
    for i in range(len(synthetic)):
        ...
        D_ref_imgs = discriminator(ref_imgs + syn_imgs)
            
        # BEGAN
        #g_loss_began = self_regularization_loss(D_ref_imgs, ref_imgs)
        g_loss_began = torch.mean(torch.abs(D_ref_imgs - (ref_imgs + syn_imgs)))
        ...
        g_loss = g_loss_began# + g_loss_reg#_scale
        ...
        D_ref_imgs2 = discriminator(ref_imgs.detach() + syn_imgs)
        ...
        d_loss_fake = torch.mean(torch.abs(D_ref_imgs2 - (ref_imgs.detach() + syn_imgs)))
